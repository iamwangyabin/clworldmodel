...
"""Failure-preserving AWM-AutoRoute boundary continuation orchestration.
...
def inspect_resume(checkpoint: Path, config: dict, protocol: str) -> dict:
    ...
    if (config["continual_method"] != "evolving_atomic_rssm_adaptive_compression_shared_heads_autoroute_arrow"
            ...
            or original != config or config.get("task_route_inference") != "two_frame_probability_reconstruction"):
        raise ValueError("Resume cannot change AWM-AutoRoute seed, protocol, budget or resolved config")
