...
"""CUDA recovery smoke on an immutable D-AutoRoute boundary checkpoint.
...
def verify_mixed_routes(wm, bank, config, next_task):
    ...
    dummy = getattr(config.esc.env_configs[0].get_function(), "dummy_previous_action", 0)
    ...
    with train._preserve_training_rng_state():
        for training in (False, True):
            wm.train(training)
            router = EpisodeReconstructionRouter(tuple(range(next_task + 1)))
            ...
            nz, nh, actions = _routed_policy_step(wm, policy, router, frames, z, h, previous,
                                                reset, stochastic=False, dummy_previous_action=dummy)
            ...
            for _ in range(4):
                nz, nh, actions = _routed_policy_step(wm, policy, router, frames, nz, nh, previous,
                                                    reset, stochastic=True, dummy_previous_action=dummy)
