...
- `run_evolving_atomic_rssm.py`: from-scratch three-task Evolving-Core Atomic
  ...
  `--task0-profile fixed_v1` to reproduce the original `2e-4` protocol. The
  separately named `--behavior-profile shared_fastkan_stable` composition uses
  Shared-Frozen-Down Q/F/P mechanisms and one replay-rehearsed FastKAN Actor
  and Critic across all three task routes without adding behavior updates.
- `smoke_evolving_atomic_rssm.py`: target-CUDA, production-shaped synthetic
  ...
  environment interaction and is evidence of execution correctness only. With
