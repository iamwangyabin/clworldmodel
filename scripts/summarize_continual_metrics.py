...
def build_raw_run_report(run_dir: Path) -> dict[str, Any]:
    """Build raw-return retention, keeping the final revisit as its own endpoint."""
    ...
    log_path = run_dir / "train.log"
    final_path = run_dir / "final_evaluation.json"
    evaluations = _parse_periodic_evaluations(
        log_path, [float(task["rew_scale"]) for task in tasks]
    )
    final = _json(final_path)
    ...
    return {
        ...
        "evaluation_protocol": {
            ...
            "episode_count_mode": config.get("evaluation_episode_count_mode", "legacy"),
            "episodes_per_task": 16,
            "acquisition_cohort": "fixed_validation",
            "final_cohort": final["seed_cohort"],
            "cohorts_paired": False,
            "caveat": (
                "Final-minus-acquisition estimates include evaluation-cohort sampling noise; "
                "first-pass retention uses matched validation cohorts."
            ),
        ...
        "paper_comparability": {
            "direct_published_table_comparison": False,
            "reason": (
                "New inference/evaluation/training protocol; no external normalization anchors applied."
            ),
        },
        "resource_accounting": _resource_accounting(run_dir),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--raw", action="store_true", help="Return-unit retention without Atari normalization"
    )
