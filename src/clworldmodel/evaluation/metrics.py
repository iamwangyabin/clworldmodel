...
def raw_retention_metrics(
    ...
    """Compute return-unit retention without Atari normalization constants."""

    scores = _matrix(raw_returns, name="raw_returns")
    ends = _task_end_rows(
        acquisition_rows, row_count=len(scores), task_count=len(scores[0])
    )
    if len(ends) != len(scores[0]) or not ends[-1] <= final_row < len(scores):
        raise ValueError(
            "Raw retention needs an acquisition row for every task and a later final row"
        )
    forgetting = [
        scores[end][task] - scores[final_row][task]
