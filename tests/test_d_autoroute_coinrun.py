"""CoinRun AWM-AutoRoute v4 contracts; fixed tensors/mocks, no simulator or updates."""
...
from test_reconstruction_router import vendor_available
if vendor_available:
    from test_reconstruction_router import Config, np, torch, train, trajectory
...
@unittest.skipUnless(vendor_available, "requires pinned vendor dependencies")
class CoinRunProtocolTests(unittest.TestCase):
    def test_fixed_five_seeds_541_epochs_and_no_atari_parameter_leak(self):
        ...
        for index, seed in enumerate(SEEDS):
            ...
            self.assertEqual(config.task_route_inference_version, 4)
    ...
    def test_full_report_keeps_540_and_541_separate_without_normalization(self):
        ...
        with TemporaryDirectory() as directory:
            ...
            (root / "launch.json").write_text(json.dumps({
                "method": "AWM-AutoRoute", "protocol": D_AUTOROUTE_COINRUN_PROTOCOL,
